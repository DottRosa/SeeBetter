 ...
 *
 * Filtro di daltonizzazione deuteranope
 */
public class DaltonizeDeuteranope extends CameraFilter implements DaltonizeInterface{
     ...
     */
    public DaltonizeDeuteranope(Context context) {
        super(context);
        ...
        switch (SettingsManager.currentMethod) {
            case COLOR: //Filtro di modifica del colore
                program = MyGLUtils.buildProgram(context, Filters.VERTEX, Filters.DEUTERANOPE_COLOR_DALTONIZATION_SHADER);
                break;

            case EDGE:  //Filtro della edge detection
                program = MyGLUtils.buildProgram(context, Filters.VERTEX, Filters.DEUTERANOPE_EDGE_SHADER);
                break;

            case BOTH:  //Filtro che unisce colore ed edge detection
                program = MyGLUtils.buildProgram(context, Filters.VERTEX, Filters.DEUTERANOPE_BOTH_DALTONIZATION_SHADER);
                break;
        }
    }
